<?php
/*
Plugin Name: Cozmeena Extended Shipping plugin
Description: Calculates shipping charge per order based on the most expensive shipping class and add a per item fee
Version: 1.0.0 (9/28/14)
Author: Sarah Maris for Cozmeena.com (Adapted from WooThemes Local Delivery and Fixed Rate shipping)
*/
...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
	...
	function coz_shipping_method_init() {
		if ( ! class_exists( 'WC_Shipping_Flat_Rate_Cozmeena' ) ) {
			class WC_Shipping_Flat_Rate_Cozmeena extends WC_Shipping_Method {
	...
	function __construct() {
		$this->id 						= 'cozmeena_flat_rate';
		$this->method_title 			= __( 'Cozmeena Flat Rate', 'woocommerce' );
		$this->cozmeena_flat_rate_option 		= 'woocommerce_cozmeena_flat_rates';
		$this->method_description 		= __( 'Cozmeena Flat rates let you define a shipping charge per order based on the most expensive shipping class and add a per item fee.', 'woocommerce' );

		add_action( 'woocommerce_update_options_shipping_' . $this->id, array( $this, 'process_admin_options' ) );
		add_action( 'woocommerce_update_options_shipping_' . $this->id, array( $this, 'process_cozmeena_flat_rates' ) );
	...
	function init_form_fields() {
		...
		$this->form_fields = array(
			...
			'title' => array(
							...
							'default'		=> __( 'Cozmeena Flat Rate', 'woocommerce' ),
							'desc_tip'		=> true
						),
			'availability' => array(
			...
			'additional_costs' => array(
							'title'			=> __( 'Costs per Shipping Class', 'woocommerce' ),
							'type'			=> 'title',
							'description'   => __( 'List base cost per shipping class and additional cost per item. Base rate for the order will be the highest shipping class base rate in order', 'woocommerce' )
						),				
	...
	function calculate_shipping( $package = array() ) {
		...
		$shipping_total = $this->coz_shipping( $package );
...
/**
 * coz_shipping function.
*/
	function coz_shipping( $package ) {
		$cost 	= null;

		if ( sizeof( $this->cozmeena_flat_rates ) > 0 ) {

			$found_shipping_classes = $this->find_shipping_classes( $package );

			// Find most expensive class (if found)
			foreach ( $found_shipping_classes as $shipping_class => $products ) {
				if ( isset( $this->cozmeena_flat_rates[ $shipping_class ] ) ) {
					if ( $this->cozmeena_flat_rates[ $shipping_class ]['cost'] > $cost ) {
						$cost 	= $this->cozmeena_flat_rates[ $shipping_class ]['cost'];
					}
				} else {
		...
		foreach ( $package['contents'] as $item_id => $values ) {
			...
			if ( $values['quantity'] > 0 && $_product->needs_shipping() ) {
				...
				if ( isset( $this->cozmeena_flat_rates[ $shipping_class ] ) ) {
					$fee	= $this->get_fee( $this->cozmeena_flat_rates[ $shipping_class ]['fee'], $_product->get_price() );
					$matched = true;
				} elseif ( $this->cost !== '' ) {
				...
				$item_costs = $item_costs + ( $fee * $values['quantity'] );
			}
		}

		if ( $matched ) {
			return $item_costs + $cost;
		} else {
			return 0;
		}
	}	
	...
	function generate_additional_costs_table_html() {
		...
		<tr valign="top">
			...
			<td class="forminp" id="<?php echo $this->id; ?>_cozmeena_flat_rates">
				<table class="shippingrows widefat" cellspacing="0">
					<thead>
						<tr>
							<th class="check-column"><input type="checkbox"></th>
							<th class="shipping_class"><?php _e( 'Shipping Class', 'woocommerce' ); ?></th>
							<th><?php _e( 'Base cost per order', 'woocommerce' ); ?> <a class="tips" data-tip="<?php _e( 'Per order shipping cost, excluding tax. Base cost for order is the base cost from the most expensive shipping class in the order', 'woocommerce' ); ?>">[?]</a></th>
							<th><?php _e( 'Shipping fee per item', 'woocommerce' ); ?> <a class="tips" data-tip="<?php _e( 'Fee excluding tax. Enter an amount, e.g. 2.50, or a percentage, e.g. 5%.', 'woocommerce' ); ?>">[?]</a></th>
						</tr>
					</thead>
					<tbody class="cozmeena_flat_rates">
						<tr>
							<td></td>
							<td class="cozmeena_flat_rate_class"><?php _e( 'Any class', 'woocommerce' ); ?></td>
						...
						<?php
						$i = -1;
						if ( $this->cozmeena_flat_rates ) {
							foreach ( $this->cozmeena_flat_rates as $class => $rate ) {
								$i++;

								echo '<tr class="cozmeena_flat_rate">
									<th class="check-column"><input type="checkbox" name="select" /></th>
									<td class="cozmeena_flat_rate_class">
			   	...
			   	<script type="text/javascript">
					jQuery(function() {
						...
						jQuery('#<?php echo $this->id; ?>_cozmeena_flat_rates').on( 'click', 'a.add', function(){

							var size = jQuery('#<?php echo $this->id; ?>_cozmeena_flat_rates tbody .cozmeena_flat_rate').size();
							...
							</tr>').appendTo('#<?php echo $this->id; ?>_cozmeena_flat_rates table tbody');

							return false;
						});

						// Remove row
						jQuery('#<?php echo $this->id; ?>_cozmeena_flat_rates').on( 'click', 'a.remove', function(){
							var answer = confirm("<?php _e( 'Delete the selected rates?', 'woocommerce' ); ?>");
							if (answer) {
								jQuery('#<?php echo $this->id; ?>_cozmeena_flat_rates table tbody tr th.check-column input:checked').each(function(i, el){
	...
	/**
	 * process_cozmeena_flat_rates function.
	 *
	 * @access public
	 * @return void
	 */
	function process_cozmeena_flat_rates() {
		// Save the rates
		$cozmeena_flat_rate_class = array();
		$cozmeena_flat_rate_cost = array();
		$cozmeena_flat_rate_fee = array();
		$cozmeena_flat_rates = array();

		if ( isset( $_POST[ $this->id . '_class'] ) ) $cozmeena_flat_rate_class = array_map( 'wc_clean', $_POST[ $this->id . '_class'] );
		if ( isset( $_POST[ $this->id . '_cost'] ) )  $cozmeena_flat_rate_cost  = array_map( 'stripslashes', $_POST[ $this->id . '_cost'] );
		if ( isset( $_POST[ $this->id . '_fee'] ) )   $cozmeena_flat_rate_fee   = array_map( 'stripslashes', $_POST[ $this->id . '_fee'] );

		// Get max key
		$values = $cozmeena_flat_rate_class;
		ksort( $values );
		$value = end( $values );
		$key = key( $values );

		for ( $i = 0; $i <= $key; $i++ ) {
			if ( ! empty( $cozmeena_flat_rate_class[ $i ] ) && isset( $cozmeena_flat_rate_cost[ $i ] ) && isset( $cozmeena_flat_rate_fee[ $i ] ) ) {

				$cozmeena_flat_rate_cost[ $i ] = wc_format_decimal( $cozmeena_flat_rate_cost[$i] );

				if ( ! strstr( $cozmeena_flat_rate_fee[$i], '%' ) ) {
					$cozmeena_flat_rate_fee[ $i ] = wc_format_decimal( $cozmeena_flat_rate_fee[$i] );
				} else {
					$cozmeena_flat_rate_fee[ $i ] = wc_clean( $cozmeena_flat_rate_fee[$i] );
				}

				// Add to flat rates array
				$cozmeena_flat_rates[ urldecode( sanitize_title( $cozmeena_flat_rate_class[ $i ] ) ) ] = array(
					'cost' => $cozmeena_flat_rate_cost[ $i ],
					'fee'  => $cozmeena_flat_rate_fee[ $i ],
				);
			}
		}

		update_option( $this->cozmeena_flat_rate_option, $cozmeena_flat_rates );
	...
	/**
	 * get_cozmeena_flat_rates function.
	 *
	 * @access public
	 * @return void
	 */
	function get_cozmeena_flat_rates() {
		$this->cozmeena_flat_rates = array_filter( (array) get_option( $this->cozmeena_flat_rate_option ) );
	}

}	
	}
}	
	add_action( 'woocommerce_shipping_init', 'coz_shipping_method_init' );
 
	function add_coz_shipping_method( $methods ) {
		$methods[] = 'WC_Shipping_Flat_Rate_Cozmeena';
		return $methods;
	}
 
	add_filter( 'woocommerce_shipping_methods', 'add_coz_shipping_method' );
}
