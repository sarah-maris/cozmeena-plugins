<?php
/*
Plugin Name: Pending Cozemena Registration Notification
Description: Send email notifications to the admin whenever a new registration is submitted for review
Author: Sarah Maris adapted from Razvan Horeanga's plugin at: http://lifeofadesigner.com/wordpress/email-notification-pending-submission-plugin/
Version: 2.0
*/
...
if ($new_status == 'pending' && user_can($post->post_author, 'edit_cozregistrys') && !user_can($post->post_author, 'publish_cozregistrys')) {
...
// Notifiy Contributor that Admin has published their post
...
else if ($old_status == 'pending' && $new_status == 'publish' && user_can($post->post_author, 'edit_cozregistrys') && !user_can($post->post_author, 'publish_cozregistrys')) {
    $username = get_userdata($post->post_author);
    $url = get_permalink($post->ID);
	$subject = "Your Cozmeena International Registry submission is now live:" . " " . $post->post_title;
 ...
 add_filter( "radio-buttons-for-taxonomies-no-term-wool_type", "__return_FALSE" ); 
 add_filter( "radio-buttons-for-taxonomies-no-term-wool_color", "__return_FALSE" ); 
